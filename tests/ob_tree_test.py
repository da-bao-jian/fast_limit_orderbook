...
def test_insert_order_function(lobtree_instance, single_order_instance_factory):
    lobtree_instance.insert_order(single_order_instance_factory(
        price=10000, id=101011, order_type='limit', side='bid',  timestamp=time.time(), size=1000, ))
    lobtree_instance.insert_order(single_order_instance_factory(
        price=10002, id=101012, order_type='limit', side='bid',  timestamp=time.time(), size=1000, ))
    lobtree_instance.insert_order(single_order_instance_factory(
        price=10001, id=101013,  order_type='limit', side='bid', timestamp=time.time(), size=1000, ))
    lobtree_instance.insert_order(single_order_instance_factory(
        price=10001, id=101014, order_type='limit', side='bid',  timestamp=time.time(), size=1000, ))
    # test tree levels
    ...
    with pytest.raises(ValueError):
        lobtree_instance.insert_order(single_order_instance_factory(
            price=10010, id=101011, order_type='limit', side='bid', timestamp=time.time(), size=1000,))
    assert lobtree_instance.order_ids[101011].price == 10000
...
def test_update_order(lob_instance):
    ...
    if order_need_update.is_bid():
        ...
        lob_instance.update_order(
            order_need_update.id, 'bid', new_price=new_price_level, change_size=False)
        assert lob_instance.bid.max_price != 20000
        ...
        lob_instance.update_order(
            order_need_update.id, 'ask', new_price=new_price_level, change_size=False)
        assert lob_instance.ask.max_price != 20000
...
def test_market_order(lob_instance, single_order_instance_factory):
    ...
    * there's a fourth scenario: market order eats up all the liquidity on the book. This is exetremely unlikely in real world, therefore not tested here
    '''
    # 1)
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9999, id=111014, order_type='limit', side='ask', timestamp=time.time(), size=1000))
    lob_instance.process_order(single_order_instance_factory(
        price=None, id=111011, order_type='market', side='bid', timestamp=time.time(), size=333))
    # if order size == market order size
    assert lob_instance.ask.limit_levels[9999].size == 1000 - 333
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9998, id=111015, order_type='limit', side='ask', timestamp=time.time(), size=333))
    lob_instance.process_order(single_order_instance_factory(
        price=None, id=111011, order_type='market', side='bid', timestamp=time.time(), size=333))
    ...
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9999.5, id=111015, order_type='limit', side='ask', timestamp=time.time(), size=1000))
    lob_instance.process_order(single_order_instance_factory(
        price=None, id=111011, order_type='market', side='bid', timestamp=time.time(), size=1000))
    ...
    assert lob_instance.ask.limit_levels[9999.5].size == 1000 - \
        (1000 - (1000 - 333))
    ...
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9999.5, id=111016, order_type='limit', side='ask', timestamp=time.time(), size=1))
    assert lob_instance.ask.price_tree[9999.5] == 2
    # 3)
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9999, id=111014, order_type='limit', side='ask', timestamp=time.time(), size=1))
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9998, id=111017, order_type='limit', side='ask', timestamp=time.time(), size=1))
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9998, id=111018, order_type='limit', side='ask', timestamp=time.time(), size=1))
    lob_instance.ask.insert_order(single_order_instance_factory(
        price=9997, id=111019, order_type='limit', side='ask', timestamp=time.time(), size=1))
    lob_instance.process_order(single_order_instance_factory(
        price=None, id=111011, order_type='market', side='bid', timestamp=time.time(), size=2))
    ...
    lob_instance.process_order(single_order_instance_factory(
        price=None, id=111011, order_type='market', side='bid', timestamp=time.time(), size=1.5))
    assert 9998 not in lob_instance.ask.limit_levels
    assert 9998 not in lob_instance.ask.price_tree
